...
-- | Read comma separated coordinates (x, y)
...
readDoubleXYZ bs = V2 (read $ B8.unpack x) (read $ B8.unpack y)
  where
    (x, y) = case B8.split ',' bs of
      x' : y' : _ -> (x', y')
...
showSample =
  map (\(t, V2 y z) -> B8.pack $ intercalate "," $ map show [t, y, z])
