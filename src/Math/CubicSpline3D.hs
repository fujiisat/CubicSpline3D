...
type FloatVector v f
  = (R1 v, Functor v, Floating f, V.Unbox f, V.Unbox (v f), Num (v f))
...
spline points = Spline a b c d ts
  where
    ...
    v        = V.zipWith4 (\tp tm pp pm -> 6 *^ ((pp ^/ tp) - (pm ^/ tm)))
                          (V.tail dt)
                          dt
                          (V.tail dp)
                          dp
    ...
    c   = V.zipWith4 (\tc pc up um -> pc ^/ tc - tc *^ (2 *^ um + up) ^/ 6)
                     dt
                     dp
                     (V.tail u)
                     u
